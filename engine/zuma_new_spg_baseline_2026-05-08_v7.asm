...
.ues_x_ok:
    ...
    ; Y: skip когда render_y < 8 (hardware-glitch top strip canvas).
    ; trackY>=16: normal variant, render_y = trackY-8 >= 8.
    ; trackY 8..15: skip (render_y был бы 0..7 = glitch zone).
    XOR A
    LD (TmpExpYShifted), A
    LD HL, (TmpHemY)
    LD DE, 16
    ...
    SBC HL, DE
    BIT 7, H
    JP NZ, .ues_next                           ; trackY < 16 → skip (top glitch zone)
    LD HL, (TmpHemY)
    LD DE, 8
    AND A
    SBC HL, DE
