...
EXPLOSION_HIDE_AT   EQU 1                ; с какого ExplodingFrame DMA-шар скрывается (с самого начала, рисуем destroy через TSU)
DESTROY_BASE_TNUM   EQU 2560             ; page 11 (#B) carpet row 40, normal variant. Frame N TNUM = base + N*2.
DESTROY_SHIFTED_OFFSET EQU 128           ; +2 carpet rows (= 64*2 cells) для shifted-up variant (используется при trackY<8)
NUM_DESTROY_FRAMES  EQU 7                ; frames в destroy_gfx.bin
...
; ================================================================
; ПЕРЕМЕННЫЕ DMA/CHAIN-PREV — выделенная зона #4000-#5FFF в slot 1 page 5.
; До main0.bin (#6000-#7FFF). Большие массивы здесь не straddle slot 1/2
; boundary при #8000 — shift кода в #6000+ их не двигает. Все DS = uninitialized
; на boot; InitGame явно зануляет ChainPrevValid[A/B], остальные пишутся per-frame.
; Освобождает slot 2 part от data-pressure → код может расти без layout corruption.
; ================================================================
    SLOT 1 : PAGE 5
    ORG #4000
...
; freeze (текст GAME OVER — TODO; пока просто стоп).
GameState:       DB 0
...
Entry:
    ...
    ; Stack в slot 3 (= 0xC800..0xFFFE = 14KB safe zone, не пересекается с TrackData).
    ; Раньше был SP=#BFFF — стек растил ВНИЗ в TrackData (0x969C..0xBFFF) и затирал
    ; точки 2643..2648 → glitch у головного шара ровно на (58, 103) (= "false killzone V7").
    LD SP, #FFFE

    ; --- Canary marker: 3 bytes "ZUM" at 0xC800 (= start of safe stack zone).
    ; Если stack overflowed past 0xC800, canary будет затёрт. RenderFrame проверяет.
    LD A, 'Z' : LD (#C800), A
    LD A, 'U' : LD (#C801), A
    LD A, 'M' : LD (#C802), A
...
UpdateGame:
    LD A, (GameState)
    OR A
    JP NZ, .ug_not_playing            ; state 1/2 — отдельные пути
    ...
    ; TEMP REVERT: subdivider /2 вернён для диагностики trail-bug в DMA chain blit.
    ; Если без subdivider trail visible (= motion 1 px/frame), а с subdivider — нет
    ; (= motion 0.5 px/frame), root cause = pre-existing render race triggered by speed.
    LD A, (FrameCounter)
    AND 1
...
.canary_ok:
    ...
    ; DEBUG pixel удалён — PAGE3 теперь зарезервирован под TrackData spillover (page #60).
    ; Если нужна диагностика — использовать дамп памяти (#C000+ читает TrackData
    ; spillover, переменные доступны напрямую через addresses в .lst).

    ; TSU writes сгруппированы early — в vblank, до DMA blits.
...
; ================================================================
; DIAGNOSTIC GRID — пишет 60 TSU destroy-спрайтов в DESC_CHAIN0 в виде сетки 6×10.
; Чтобы увидеть где TSU layer 1 рендерится корректно по всему экрану.
; Шаг X = 56 px (= 6 cols × 56 = 336 — ~центр canvas 360).
; Шаг Y = 28 px (= 10 rows × 28 = 280 — ~центр canvas 288).
; Frame 0, SPAL=2 (синий).
; ================================================================
DiagnosticGrid:
    DI
    LD BC, FMADDR : LD A, FM_EN : OUT (C), A
    LD HL, DESC_CHAIN0
    LD B, 0                                   ; row counter (0..9)
.dg_row:
    LD C, 0                                   ; col counter (0..5)
.dg_col:
    ; Y = row * 28
    LD A, B
    LD D, A : ADD A, A : ADD A, A : ADD A, A  ; A = row*8
    ADD A, A                                  ; A = row*16
    ADD A, D                                  ; A = row*16 + row = row*17? OEPS
    ; нужно row*28. Простой способ: row*32 - row*4.
    LD A, B
    ADD A, A : ADD A, A : ADD A, A : ADD A, A : ADD A, A   ; row*32
    LD D, A
    LD A, B
    ADD A, A : ADD A, A                       ; row*4
    LD E, A
    LD A, D
    SUB E                                     ; A = row*32 - row*4 = row*28
    LD (HL), A : INC HL                       ; Y_L
    LD (HL), SPACT + SPSIZ16                  ; Y_H=0, ACT, SIZE16
    INC HL

    ; X = col * 40 + 28 (max col=5: X = 228, < 256 — fits в byte без X[8])
    LD A, C
    ADD A, A : ADD A, A : ADD A, A            ; col*8
    LD D, A
    LD A, C
    ADD A, A : ADD A, A : ADD A, A : ADD A, A : ADD A, A   ; col*32
    ADD A, D                                  ; col*32 + col*8 = col*40
    ADD A, 28                                 ; +28 offset (центр 6 колонок: 28..228)
    LD (HL), A : INC HL                       ; X_L
    LD (HL), SPSIZ16                          ; X_H=0, SIZE16
    INC HL

    ; TNUM = DESTROY_BASE_TNUM = 2560 (frame 0)
    LD (HL), LOW(DESTROY_BASE_TNUM)
    INC HL
    LD A, HIGH(DESTROY_BASE_TNUM)
    AND #0F
    OR #20                                    ; SPAL=2 (синий) в high nibble
    LD (HL), A
    INC HL
...
.is_clr2:
    ...
    ; --- TrackData splits across slot 2 (page 2) и slot 3 (page #60). 12386 bytes
...
; ================================================================
ComputeFrogAngle:
    ; dx = (SmoothMouseX + 8) - (FrogX + 32) = SmoothMouseX - FrogX - 24
    ; (центр курсора 16×16 относительно центра жабы 64×64). Используется
    ; SmoothMouseX/Y, не raw — иначе кадровое jitter перекидывает FrogAngle.
    LD HL, (SmoothMouseX)
    LD DE, (FrogX)
    AND A
    SBC HL, DE
...
.dx_pos:
    ...
    ; dy = (SmoothMouseY + 8) - (FrogY + 32) = SmoothMouseY - FrogY - 24
    LD HL, (SmoothMouseY)
    LD DE, (FrogY)
    AND A
    SBC HL, DE
...
UpdateFrogSprite:
    ...
    LD DE, (FrogY)
    LD (HL), E : INC HL
    LD (HL), %01101110              ; SPSIZ64+ACT+SPLEAP — следующие sprites в layer 1 (над T0+canvas)
    INC HL
...
.scb_off_set:
    ...
    ; Обнулить Shot2/ExplodingFrame/ExplodingMarker для нового tail-слота.
    ; Иначе stale значение от прошлой жизни этого слота (после shift'ов cascade close)
    ; вызывает ложный match-3 / преждевременную финализацию свежеспавненного шара,
    ; даже если игрок не стрелял.
    LD A, (Chain0_SlotsLen)
    LD H, 0 : LD L, A
    LD DE, Chain0_Shot2
    ADD HL, DE
...
.icb_avg_d_pos:
    ...
    ; shift_right ExplodingFrame — иначе финализация после EXPLOSION_FRAMES
    ; пишет GAP в неправильный slot и удаляет non-exploding шар по соседству.
    POP BC : PUSH BC
    LD A, (Chain0_SlotsLen)
    DEC A
    LD H, 0 : LD L, A
    ...
    ; shift_right ExplodingMarker (парный с ExplodingFrame)
    POP BC
    LD A, (Chain0_SlotsLen)
    DEC A
    LD H, 0 : LD L, A
...
.icb_no_shift:
    ...
    ; Новый шар не exploding — обнуляем ExplodingFrame[idx] и ExplodingMarker[idx]
    ; (после shift_right на этом месте может оказаться унаследованный мусор от старого slot[idx-1]).
    LD A, (TmpInsertIdx)
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
    ADD HL, DE
...
.icb_no_head_comp:
    ...
    ; ChainFreezeCounter НЕ ставится при insert — юзер хочет чтобы хвост не
    ; останавливался, а голова ускоренно ушла вперёд (= освободила место).
    ; Без freeze: offsets[head-side] decay -CS→0 параллельно с natural hsub++ →
    ; head net advance 2 cells за CELL_SIZE кадров (вместо 1 cell с freeze).
    ; Tail (idx..end) после shift+HSA+1 остаётся на абсолютной позиции, spawn
    ; работает по таймеру независимо. Cascade-close в DoGapStep продолжает
    ; ставить freeze (визуальный rollback нужен при schлопывании цепи).
    RET
...
.dm3_l:
    ...
    ; Если сосед уже exploding — break (не расширять run на удаляющийся шар)
    LD A, B
    DEC A
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
...
.dm3_r:
    ...
    ; Если сосед уже exploding — break.
    LD A, C
    INC A
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
...
.dgs_stop_found:
    ...
    ; ExplodingFrame shift — иначе финализация других exploding слотов
    ; промахивается мимо своих позиций после compaction соседнего STOP.
    LD A, (TmpGapIdx)
    INC A
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
...
.dgs_casc_found:
    ...
    ; ExplodingFrame shift — финализация других exploding слотов после CASCADE close.
    LD A, (TmpGapIdx)
    INC A
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
...
; ================================================================
UpdateStallByGap:
    ; ChainStalled = 1 ТОЛЬКО при наличии GAP-cells (= match-3 в процессе схлопывания).
    ; Раньше также stall'илось при non-zero offsets («анимация в процессе») — это давало
    ; +32 кадра паузы поверх ChainFreezeCounter после ЛЮБОГО insert (без match), т.к.
    ; head комп offsets[0..idx-1]=-32 декаит 32 кадра, всё это время stall=1, freeze
    ; counter даже не декрементируется. Итого суммарная пауза ~1.1 сек = жалоба
    ; «трек останавливается при попадании». Python `update_stall` просто `s.chain_stalled=0`
    ; всегда; ChainFreezeCounter сам по себе уже даёт нужную паузу 32 кадра.
    LD A, (Chain0_SlotsLen)
    OR A
...
.mc_no_freeze:
    ...
    CP CELL_SIZE
    JR C, .mc_save_sub
    ; Sub wraps to 0. Обычно HSA++. Если HSA уже на cap (TRACK_NUM_SLOTS-1) —
    ; head шар «вылетает» в killzone: shift_left arrays + SlotsLen--. Иначе chain
    ; залипает на конце track когда длина почти max — head не может advance.
    XOR A
    LD (Chain0_HeadSub), A
    LD A, (Chain0_HeadSlotAbs)
    INC A
...
; ================================================================
; MOVE CHAIN ABSORB — вариант MoveChain'а для Game-Over state 1.
; При HSA on cap И hsub wraps → НЕ clamp HSA, а CALL AbsorbHead — head шар
; «вылетает» в killzone, цепь укорачивается на 1, остальные шары сдвигаются
; так что их track-позиции непрерывны (HSA НЕ декрементируем — shift сам
; компенсирует, см. ниже). Используется в UpdateGame.ug_state1.
; ================================================================
MoveChainAbsorb:
    LD A, (CascadeState) : OR A : RET NZ
    LD A, (ChainStalled) : OR A : RET NZ
    LD A, (ChainFreezeCounter)
    OR A
    JR Z, .mca_no_freeze
    DEC A : LD (ChainFreezeCounter), A
    RET
.mca_no_freeze:
    LD A, (Chain0_HeadSub)
    INC A
...
; ================================================================
; ABSORB HEAD — shift_left Chain0_Slots/SlotOffsets/Shot2/ExplodingFrame/
; ExplodingMarker [1..len-1] → [0..len-2]; SlotsLen-=1; AbsorbCounter+=1.
; HSA НЕ изменяем. Если SlotsLen уже 0 — RET.
; ================================================================
AbsorbHead:
    LD A, (Chain0_SlotsLen)
    OR A
    ...
    LD (Chain0_SlotsLen), A
    OR A
    JR Z, .ah_count                  ; new len=0 → нечего сдвигать
    LD C, A : LD B, 0                ; BC = count = new SlotsLen
    PUSH BC
    LD HL, Chain0_Slots + 1
    LD DE, Chain0_Slots
    LDIR
    POP BC : PUSH BC
    LD HL, Chain0_SlotOffsets + 1
    LD DE, Chain0_SlotOffsets
    LDIR
    POP BC : PUSH BC
    LD HL, Chain0_Shot2 + 1
    LD DE, Chain0_Shot2
    LDIR
    POP BC : PUSH BC
    LD HL, Chain0_ExplodingFrame + 1
    LD DE, Chain0_ExplodingFrame
    LDIR
    POP BC
    LD HL, Chain0_ExplodingMarker + 1
    LD DE, Chain0_ExplodingMarker
    LDIR
.ah_count:
    LD A, (AbsorbCounter)
...
; ================================================================
; CHECK HEAD AT KILLZONE — Manhattan(head_pos, KzCenter) < 16 ?
; Out: A = 1 если да (= триггер Game Over), A = 0 иначе.
; Использует ComputeSlotXY(0) → TmpHemX/TmpHemY, затем подставляет
; KzCenter в TmpBallCX/TmpBallCY и вызывает ManhattanDistToBall.
; Clobbers A,BC,DE,HL,TmpHemX/Y,TmpBallCX/Y,TmpHemSlotIdx.
; ================================================================
CheckHeadAtKillzone:
    LD A, (Chain0_SlotsLen)
    OR A
    JR Z, .chk_no                    ; пустая цепь — не trigger
    XOR A                            ; head idx = 0
    ...
    LD HL, (KzCenterY)
    LD (TmpBallCY), HL
    CALL ManhattanDistToBall         ; HL = Manhattan
    LD A, H
    OR A
    JR NZ, .chk_no                   ; HL >= 256 — точно >= 16
    LD A, L
    CP 16
    JR NC, .chk_no
    LD A, 1
...
BlitChainToShadow:
    ...
    ; BcsGoldenOff читается KzRestoreHalf в BlitKillzoneToShadow → setup обязателен
    ; даже когда chain DMA отключён (chain рисуется TSU).
    LD A, (ShadowPageBase)
...
.bpc_loop:
    ...
    ; --- Test 2b: Exploding слот → DMA скрывается, рисуется TSU destroy через UpdateExplodeSprites ---
    LD A, (TmpChainIdx)
    LD H, 0 : LD L, A
    LD DE, Chain0_ExplodingFrame
    ADD HL, DE
    LD A, (HL)
    OR A
    JP NZ, .bpc_destroy                        ; ExplodingFrame > 0 → DMA не рисует, TSU перекрывает
...
; (Bcs/TmpKz/Visible/Shadow/ChainPrev*/BcsSkipFlag/BcsCacheStruct
;  переехали в выделенную зону #4000-#5FFF в начале файла — см. там.)
...
; ================================================================
; UPDATE EXPLODE SPRITES — TSU-render destroy-frames для exploding слотов.
; Static mapping: tsu_slot = chain_idx. Если chain длина > TSU_CHAIN_SPRITES,
; explosion на дальних слотах не показывается.
; SPSIZ24 (= 24×24): halfsize=12, clamp X/Y к 0 при negative.
; ================================================================
UpdateExplodeSprites:
    DI
    LD BC, FMADDR : LD A, FM_EN : OUT (C), A
    XOR A
    LD (TmpChainIdx), A
.ues_loop:
    ...
    ; SPSIZ16: halfsize=8. Clamp X/Y к 0. Шар centered when trackX>=8.
    LD HL, (TmpHemX)
    LD DE, 8
    AND A
    SBC HL, DE
...
.ues_x_ok:
    ...
    ; Y: для trackY >= 8 — normal variant, render_y = trackY - 8 (центр at +8 = trackY).
    ; Для trackY < 8 — shifted-up variant (content rows 2..15 → output 0..13),
    ; render_y = max(0, trackY - 6) → центр content (≈row 6.5) близко к trackY.
    ; Иначе sprite в спавн-зоне рисуется на 6..8 px выше реального шара и
    ; визуально пересекается с соседним → создаёт иллюзию ложного match-3.
    LD HL, (TmpHemY)
    LD DE, 8
    AND A
    SBC HL, DE
    BIT 7, H
    JR NZ, .ues_y_shifted                      ; trackY < 8 → shifted variant
    XOR A
    LD (TmpExpYShifted), A
    LD (TmpExpY), HL                           ; render_y = trackY - 8
    JR .ues_y_done
.ues_y_shifted:
    LD A, 1
    LD (TmpExpYShifted), A
    ...
    SBC HL, DE
    BIT 7, H
    JR Z, .ues_y_shifted_ok
    LD HL, 0
.ues_y_shifted_ok:
    LD (TmpExpY), HL                           ; render_y = max(0, trackY - 6)
.ues_y_done:

    LD A, (TmpExpFrameIdx)
    ADD A, A                                   ; A = frame_idx * 2 (2 cells/frame для 16×16)
    LD E, A
    LD D, 0
    LD HL, DESTROY_BASE_TNUM
    ADD HL, DE
    LD A, (TmpExpYShifted)
    OR A
    JR Z, .ues_tnum_done
    LD DE, DESTROY_SHIFTED_OFFSET
    ADD HL, DE                                 ; +128 = переход на shifted variant (carpet rows 2..3)
.ues_tnum_done:
    LD (TmpExpTNUM), HL
...
; ================================================================
; UPDATE CHAIN TSU SPRITES — TSU descriptor для каждого живого chain ball.
; Рисует normal ball atlas (TNUM=2304+COLOR*3, SPAL=COLOR+2, SPSIZ24=24×24) на
; TSU layer 1 поверх canvas (включая killzone-зону). Заменяет HideChainSprites —
; раньше chain рисовался только в canvas через DMA, и restore golden→shadow в
; killzone-зоне затирал killzone-pixels там, где раньше был ball ("под шаром
; фон трека"). UpdateExplodeSprites потом override-ит destroy frame (16×16)
; для exploding слотов.
; Slots с GAP_MARKER или i >= SlotsLen → off-screen (Y=#2C, ACT=1).
; Static mapping: tsu_slot = chain_idx (0..TSU_CHAIN_SPRITES-1).
; ================================================================
UpdateChainTSUSprites:
    DI
    LD BC, FMADDR : LD A, FM_EN : OUT (C), A
    XOR A
    LD (TmpChainIdx), A
.uts_loop:
    ...
    LD HL, Chain0_SlotsLen
    CP (HL)
    JP NC, .uts_offscreen                      ; i >= SlotsLen → off-screen
    ...
    LD A, (HL)
    CP NUM_BALL_COLORS
    JP NC, .uts_offscreen                      ; GAP_MARKER → off-screen
    LD (TmpChainColor), A
...
.uts_y_ok:
    ...
    ; descriptor addr = DESC_CHAIN0 + i*6
    LD A, (TmpChainIdx)
    LD H, 0 : LD L, A
    ADD HL, HL
    LD D, H : LD E, L
    ...
    LD (HL), E : INC HL                        ; Y_L
    LD A, D : AND 1 : OR SPACT + SPSIZ24
    LD (HL), A : INC HL                        ; Y_H + ACT + SIZE24

    LD DE, (TmpChainX)
    LD (HL), E : INC HL                        ; X_L
    LD A, D : AND 1 : OR SPSIZ24
    LD (HL), A : INC HL                        ; X_H + SIZE24

    ; TNUM = 2304 + COLOR*3 (24×24 ball stride 3 cells)
    LD A, (TmpChainColor)
    LD B, A
    ADD A, A
    ADD A, B                                   ; A = COLOR*3
    LD (HL), A : INC HL                        ; TNUM_L (LOW(2304)=0, no carry for COLOR≤5)

    ; TNUM_H + SPAL: HIGH(2304)=9, SPAL=COLOR+2
    LD A, (TmpChainColor)
    ADD A, 2
    SLA A : SLA A : SLA A : SLA A
    OR 9
...
.uts_offscreen:
    ...
    ADD HL, DE
    LD (HL), #2C : INC HL                      ; Y_L
    LD (HL), SPACT+SPSIZ24+1 : INC HL          ; Y_H + ACT + SIZE24 + Y[8]=1
    LD (HL), 0 : INC HL                        ; X_L
    LD (HL), SPSIZ24 : INC HL                  ; X_H + SIZE24
    LD (HL), 0 : INC HL                        ; TNUM_L
    LD (HL), 0                                 ; TNUM_H + SPAL
...
UpdateCursorSprite:
    ...
    ; Cursor рисуется на RAW позиции — мгновенный отклик мыши.
    ; (Smooth применяется только для FrogAngle, где kempston jitter раздражает,
    ; а 3-4 кадра latency незаметны.)
    LD DE, (MouseAbsY)
    LD (HL), E : INC HL                         ; Y_L
...
; ================================================================
; ================================================================
; APPLY SMOOTH MOUSE — exponential moving average alpha=1/4.
;   smooth = (smooth*3 + raw) / 4
; Раз за кадр после HandleMouse. Inputs: MouseAbsX/Y (16-bit raw, 0..CURSOR_MAX).
; Outputs: SmoothMouseX/Y (16-bit), используется в ComputeFrogAngle / UpdateCursorSprite.
; Гасит ±1-2 px kempston jitter; latency ~3-4 кадра до 95% от raw.
; Источник: VDAC2 → VDC чат 2026-05-09 17:30.
; ================================================================
ApplySmoothMouse:
    ; --- X: HL = (SmoothX*3 + RawX) / 4 ---
    LD HL, (SmoothMouseX)
    LD D, H : LD E, L
    ADD HL, DE                ; HL = smooth*2
    ADD HL, DE                ; HL = smooth*3
    LD DE, (MouseAbsX)
    ADD HL, DE                ; HL = smooth*3 + raw
    SRL H : RR L
    SRL H : RR L              ; HL /= 4
    LD (SmoothMouseX), HL

    ; --- Y: HL = (SmoothY*3 + RawY) / 4 ---
    LD HL, (SmoothMouseY)
    LD D, H : LD E, L
    ADD HL, DE
    ADD HL, DE
...
.my_done:
    ...
    ; Low-pass фильтр (exp moving avg, alpha=1/4) — гасит kempston jitter ±1-2 px.
    ; smooth = (smooth*3 + raw) / 4. Применяется ВСЕГДА (даже когда raw не двигалась)
    ; — иначе при остановке курсор «застывает» на raw без сглаживания, видимый микро-jitter.
    CALL ApplySmoothMouse
...
SmoothMouseX: DW MOUSE_INIT_X ; low-pass smoothed X (init = центр жабы)
SmoothMouseY: DW MOUSE_INIT_Y ; low-pass smoothed Y
...
TmpExpFrameIdx:   DB 0  ; UpdateExplodeSprites: 0..6 destroy frame
TmpExpColor:      DB 0  ; UpdateExplodeSprites: цвет шара (для SPAL)
TmpExpX:          DW 0  ; UpdateExplodeSprites: top-left X
TmpExpY:          DW 0  ; UpdateExplodeSprites: top-left Y
TmpExpTNUM:       DW 0  ; UpdateExplodeSprites: TNUM байт-pair
TmpExpTSUSlot:    DB 0  ; UpdateExplodeSprites: индекс свободного TSU-слота для следующего descriptor'а
TmpExpYShifted:   DB 0  ; 1 если используется shifted-up variant (для trackY<8)
...
TRACK_NUM_POINTS EQU (TrackEnd - TrackOverflow + 10596 - 2) / 4  ; = 3096
    ...
    SAVEBIN "main1.bin", #8000, #4000     ; slot 2 part — 16K (TrackData first 9572 bytes)
    ; TrackData spillover в slot 3 page #60 (track_overflow.bin = bytes 9572..end of level_01.bin).
    ; SAVEBIN читает с явной PAGE #0C чтобы получить правильные bytes (sjasmplus
    ; otherwise может emit'ить в неточные positions при cross-slot INCBIN).
    SLOT 3 : PAGE #0C
    SAVEBIN "track_overflow.bin", #C000, #800   ; 2KB (2814 valid bytes + zeros padding)
    SLOT 2 : PAGE 2
